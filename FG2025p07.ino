...
#define VERSION_NUMBER  2025.07        // Version number appears in Display #1 / Credit display at start of game
#define ATTRACT_SPEECH_TIMER 300000    // (300000) Amount of time between attract mode speech callouts. 60000 = 1 minute
#define USE_STROBE 1                   // 0 = don't use backglass strobe, 1 = use backglass strobe Note: Tested with LED strobe. May be too frequent for original Bally strobe.
...
#define BALL_SAVE_SCORE 1500           // If score less than this number for one ball, player is given a chance to try again.
#define BALL_SAVE_NUM_MS 15000         // If game play less than this number of ms for one ball, player is given a chance to try again.
#define BALL_SAVES 1                   // Number of ball saves allowed to each player. Zero for no ball save.
...
#define RPU_OS_USE_GEETEOH             // If defined, program expects a Geeteoh sound board to play pre-programmed sound files.
//=========================================================================================================
...
struct Debounce ResetHits[17] = {
  ...
  {0,   0}  // SO_FLIPPER_ENABLE         16 KI RELAY (FLIPPER ENABLE)
};
...
void PlaySound(byte soundByte, unsigned long delaySound = 0) {

  // Specify number of sound to play (see GeeteohTimings list of sounds), and
  //    length of delay until sound will be played (in ms, default zero)
  ...
  if (GeeteohTimings[soundByte][1] == 1 || 
      GeeteohTimings[soundByte][1] == 8 || 
      GeeteohTimings[soundByte][1] == 9)      soundType = 1;      // Background / Stop All / Alarm 
  else if (GeeteohTimings[soundByte][0] == 1) soundType = 2;      // Voice
  ...
  if (soundType == 2 && delaySound == 0 && GTOWaitingTime < CurrentTime) {   // Play voice immediately, unless delayed or another voice playing
    RPU_PlaySoundSAndT(soundByte);
    GTOLatestSound = CurrentTime;
    GTOWaitingTime = CurrentTime + 100 * (unsigned long) max(GeeteohTimings[soundByte][2], GeeteohTimings[soundByte][3]);
    return;
  }
  ...
  SoundQueue[kGTO].length = 100 * (unsigned long) max(GeeteohTimings[soundByte][2], GeeteohTimings[soundByte][3]);
}
    ...
    if (SoundQueue[iGTO].type != 0xFF) {                                     // Look for valid sound type. 0xFF indicates empty row of queue.
      if (CurrentTime >= SoundQueue[iGTO].start) {                           // This sound is ready to be played
        if (SoundQueue[iGTO].type == 1) {                                    // Background sounds always play at requested start time
          RPU_PlaySoundSAndT(SoundQueue[iGTO].sound);
          GTOLatestSound = CurrentTime;
...
void PlaySound(byte soundByte, unsigned long delaySound = 0) {
  ...
  if (CurrentTime < CurrentSoundEffectTimer && soundByte != 10) return; // Do not interrupt a timed sound, unless it is the 15-second timer
  ...
  if (soundByte > 51)
    ...
    CurrentSoundEffectTimer = CurrentTime + (GeeteohTimings[soundByte] * 250); // 250 = 0.25 seconds
  RPU_PlaySoundSAndT(soundByte);
}
...
int RunSelfTest(int curState, boolean curStateChanged) {
  ...
  if (returnState == MACHINE_STATE_ATTRACT){
    ...
    if (kidMode) 
      ...
      ballSaves = BALL_SAVES; // Reset operator game adjustments that are changed in KidSettings()
      MingMax = MAX_HEALTH;
    }
...
int RunGamePlayMode(int curState, boolean curStateChanged) {
  ...
  if (curState==MACHINE_STATE_INIT_GAMEPLAY) {
    ...
    returnState = WizardMode(curStateChanged);
      ...
      if ((BallSaveUsed[CurrentPlayer] > 0) && (CurrentTime < (BallSaveStartTime + BALL_SAVE_NUM_MS))) {
      ...
      else if ((BallSaveUsed[CurrentPlayer] > 0) && (CurrentScores[CurrentPlayer] < InitialScore + BALL_SAVE_SCORE)) {
        BallSaveUsed[CurrentPlayer]--;
        RPU_SetLampState(LA_SAME_PLAYER_SHOOTS_AGAIN, 0);
        PlayerShootsAgain = true;
        CurrentScores[CurrentPlayer] = InitialScore;
      ...
      switch (switchHit) {
        ...
        case SW_CREDIT_BUTTON:
            if (CurrentScores[CurrentPlayer] != 0 && STOP_THE_MUSIC) {
              PlaySound(90); // Stop background music during game. Some may find this more useful than adding players during first ball.
              break;
              }
            ...
            if (switchHit==SW_DTARGET_4_A) {
              if (DTarget4Light[CurrentPlayer][0]==0) {
               ...
               DropTargetHit();
                ...
                //  PlaySound(78); // 15-second timer
                //  PlaySound(17, 100); // Switches over there, start hitting them! Was 47, 15 second warning.
                PlaySound(17);
                AddToScore(10000);
                AddToSuperBonus(1);
                DropTargetHit();
            ...
            if (switchHit==SW_DTARGET_4_B) {
              if (DTarget4Light[CurrentPlayer][1]==0) {
               ...
               DropTargetHit();
                ...
                //  PlaySound(79); // 5-second timer
                //  PlaySound(27, 100); // Look out, Flash! Was 47, 15 second warning.
                PlaySound(27); // Look out, Flash!
                AddToScore(10000);
                AddToSuperBonus(1);
              DropTargetHit();
        ...
        case SW_DTARGET_3_C:
          ...
          if (curState!=MACHINE_STATE_WIZARD_MODE) {
            ...
            PlaySound(24); // wizard sound: bong bounce down
            // My_PushToTimedSolenoidStack(SO_DTARGET_3_RESET, 15, CurrentTime);
            if (WizardState[CurrentPlayer]==4) {
              RPU_SetLampState(LA_OUTLANE_RIGHT_SPECIAL, 0);
              RPU_SetLampState(LA_OUTLANE_LEFT_SPECIAL, 0);
              OutLanesLit[CurrentPlayer] = false;
...
void DropTargetHit() {
  ...
  if (DropTargetCount==WizardGoalTargets) {
    ...
    PlaySound(254); // wizard sound: This way Flash, come on! Was 44, Emperor Ming awaits.
  }
}
...
int CountdownBonus(boolean curStateChanged) {
  ...
  if ((CurrentTime - LastCountdownReportTime) > DisplayCDBTime) {
    ...
    if (MiniBonus>0) {
      ...
      ShowMiniBonusOnLadder(MiniBonus);
      PlaySound(12); // Ascending sound
      if (MiniBonus == 0) DisplayCDBTime = 200;
      if (BonusXState[CurrentPlayer]>1 && MiniBonus==0) {
      ...
      ShowSuperBonusOnLadder(SuperBonus);
      PlaySound(12); // Ascending sound
      if (SuperBonus == 0) DisplayCDBTime = 200;
      if (BonusXState[CurrentPlayer]>1 && SuperBonus==0) {
...
int ShowMatchSequence(boolean curStateChanged) {  
  if (curStateChanged) {
    ...
    PlaySound(30, MatchDelay); // Spinner noise. Was 28. Original game used this for both match and outlane. Outlane is now a voice, and unsuitiable for an echo effect. Note, echo now removed as I couldn't get it right!
  }
...
int WizardMode(boolean curStateChanged) {
  ...
  if (curStateChanged) {
    ...
    #ifndef RPU_OS_USE_GEETEOH 
    PlaySound(48, 150); // wizard sound: miserable earthling
    #else
...
int RunExtendedSelfTest(int curState, boolean curStateChanged, unsigned long CurrentTime, byte resetSwitch, byte otherSwitch, byte endSwitch) {
  ...
  if (curState==MACHINE_STATE_TEST_TOTAL_SKILL) {
    auditNumStartByte = 0;
    auditNumStartUL = 0;
    if (curStateChanged) {
      skillNum = 0;
